...
<script lang="ts">
  ...
  import { apiGetMineGroups } from '/@/api/sys/permission';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const { userId, realName, phone } = userStore.getUserInfo || {};

      const groupNames = ref<string[]>([]);
      onMounted(async () => {
        const groupList = await apiGetMineGroups();
        for (const groupInfo of groupList) {
          groupNames.value.push(groupInfo.group_name);
        }
      });
